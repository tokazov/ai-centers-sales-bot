...
async def on_manage_train(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "Скопируйте реальные переписки с клиентами — AI-ассистент научится отвечать так же\n\n"
        "📸 <b>Фото</b>\n"
        "Фото меню, витрины, прайса — AI-ассистент распознает текст\n\n"
        "💡 Можно отправлять данные в любой момент — AI-ассистент дообучается автоматически!",
        reply_markup=kb,
    )
    await callback.answer()
...
async def on_manage_edit(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        "✏️ <b>Изменить AI-ассистента</b>\n\n"
        "Напишите что хотите изменить. Примеры:\n\n"
        "💬 <b>Стиль общения:</b>\n"
        "«Пусть AI-ассистент обращается на вы и говорит формально»\n"
        ...
        "Просто опишите — мы обновим AI-ассистента 👇",
        reply_markup=kb,
    )
    await callback.answer()
...
async def on_manage_faq(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "→ Убедитесь что AI-ассистент запущен (напишите ему /start). Если не помогло — напишите нам.\n\n"
